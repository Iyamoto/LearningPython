...
def getLinks(text, filter):
    assert type(text)==str
    links = re.findall(r'<a href="(http[^"]+)"',text)
    filtered = []
    for link in links:
        if link.find(filter)>=0:
            filtered.append(link)
    return filtered

def saveUrlsfromSerp(serpdir, outfile, c=2):
    files = []
    for i in range(c):
    ...
    f = open(outfile,'w')
    for file in files:
        filepath = os.path.join(serpdir, file)
        html = file2text(filepath)
        links = getLinks(html,'sym/root')
        for link in links:
            f.write(link+'\n')
    f.close()        
    return None
...
basedir = 'R:\\lucid\\anomaly\\1\\'
serpdir = os.path.join(basedir, 'serp')
...
#saveUrlsfromSerp(serpdir, 'urls.txt')

urls = file2list('uniqurls.txt')
urls.sort()
print(len(urls))
